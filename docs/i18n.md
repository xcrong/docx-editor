...
```bash
bun run --filter '@xcrong/docx-editor-i18n' build
bun run api:extract
...
Select `@xcrong/docx-editor-i18n` with a `patch` bump and write a one-line summary (e.g. `add <Language> (<code>) community-maintained locale`). The summary lands verbatim in the published `CHANGELOG.md`.
