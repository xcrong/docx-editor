...
Either adapter pulls in `@xcrong/docx-editor-core` and `@xcrong/docx-editor-i18n` transitively. One install is enough.
...
Run your dev server, click the file input, and pick a `.docx`: a contract, a CV, or a report with tables and headers. The editor parses it client-side and the download button writes the current state back to `.docx`. Check the result in Microsoft Word. If something renders or saves incorrectly, [file an issue](https://github.com/xcrong/docx-editor/issues) with the document.
