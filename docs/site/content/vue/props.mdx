...
Vue also has `:show-menu-bar` (default `true`) for the menu bar above the toolbar; React has no menu-bar concept. The React-only chrome props `showMarginGuides`, `marginGuideColor`, and `rulerUnit` are not available in Vue.
