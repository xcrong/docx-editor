...
Two files. The route stays a thin client shell; the editor itself lives in a separate component loaded with `dynamic()` and `ssr: false`. This is the structure of [`examples/nextjs`](https://github.com/xcrong/docx-editor/tree/main/examples/nextjs).
...
The example resolves `@xcrong/*` from the monorepo workspace, so build the packages once first:
...
- [Next.js example on GitHub](https://github.com/xcrong/docx-editor/tree/main/examples/nextjs)
