...
Remix renders routes on the server by default and the editor is browser-only, so the route gates it behind a mount check plus a `lazy()` import. This is the pattern from [`examples/remix/app/routes/_index.tsx`](https://github.com/xcrong/docx-editor/tree/main/examples/remix):
...
The example resolves `@xcrong/*` from built output, so build the workspace packages once first:

```bash
git clone https://github.com/xcrong/docx-editor.git
cd docx-editor
bun install
...
- [Remix example on GitHub](https://github.com/xcrong/docx-editor/tree/main/examples/remix)
