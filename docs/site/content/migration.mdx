...
description: "Move from @xcrong/docx-js-editor 0.x to the 1.x package set. Package-rename map, moved-symbol map, i18n restructure, license update."
...
0.x was a single package. 1.x splits it into five npm packages by concern (plus `@xcrong/nuxt-docx-editor`, a Nuxt module wrapping the Vue adapter):

| What you used in 0.x | Where it is in 1.x |
|---|---|
| The `<DocxEditor>` component, toolbar, dialogs, hooks, plugin host | `@xcrong/docx-editor-react` (or `@xcrong/docx-editor-vue`) |
| The document model and OOXML parse/serialize (`Document`, `Paragraph`, `createEmptyDocument`, ...) | `@xcrong/docx-editor-core` |
| Anything AI: the reviewer, agent UI components, the MCP server | `@xcrong/docx-editor-agents` |
| Locale strings (`pl`, `de`, `pt-BR`, ...) | `@xcrong/docx-editor-i18n` |

In practice a React app still imports from one package, `@xcrong/docx-editor-react`, which pulls in `-core` and `-i18n` for you. You only name the other packages when you import the document model, the agent toolkit, or locale data directly. The rest of this guide is the symbol-level detail behind that split.
...
3. Change every remaining `@xcrong/docx-js-editor` import to `@xcrong/docx-editor-react`.
...
The monolithic `@xcrong/docx-js-editor` split into five packages under the `@xcrong/docx-editor-*` namespace, plus the `@xcrong/nuxt-docx-editor` module.

| Old (0.x) | New (1.x) |
|---|---|
| `@xcrong/docx-js-editor` | `@xcrong/docx-editor-react` |
| `@xcrong/docx-js-editor/core` | `@xcrong/docx-editor-core` |
| `@xcrong/docx-js-editor/mcp` | `@xcrong/docx-editor-agents/mcp` |
| `@xcrong/docx-js-editor/i18n/<code>.json` | `@xcrong/docx-editor-i18n/<code>` (subpath) or named export off the root |
| `@xcrong/docx-editor-agents` (AGPL-3.0) | `@xcrong/docx-editor-agents` (Apache 2.0) |

Install the package set you actually use:

```bash
# Bare minimum for a React editor:
npm uninstall @xcrong/docx-js-editor
...
Five categories of symbols moved across packages (not just subpaths). A plain package-name swap to `@xcrong/docx-editor-react` won't resolve these. **If a symbol from this list lives in your code, point its import at the new home below; otherwise every one of them becomes a "no exported member" error.**

| Symbol(s) | Was (0.x) | Now (1.x, recommended) |
|---|---|---|
| `createEmptyDocument`, `createDocumentWithText`, `CreateEmptyDocumentOptions` | `@xcrong/docx-js-editor` | `@xcrong/docx-editor-react` or `@xcrong/docx-editor-vue` (re-exported in 1.0.1 from `-core`) |
| Document-tree types most apps reach for: `Document`, `Comment`, `Paragraph`, `Run`, `Insertion`, `Deletion`, `TrackedChangeInfo`, `Hyperlink`, `Table` | `@xcrong/docx-js-editor` | `@xcrong/docx-editor-core` (root entry surfaces them in 1.0.1) |
| Rarer document-tree types: `Image`, `Footnote`, `Endnote`, `TableRow`, `TableCell` | `@xcrong/docx-js-editor` | `@xcrong/docx-editor-core` (root entry; also on `/headless`) |
| Agent UI components: `AgentPanel`, `AgentChatLog`, `AgentComposer`, `AgentSuggestionChip`, `AgentTimeline` (and their `*Props` types) | `@xcrong/docx-js-editor` | `@xcrong/docx-editor-agents/react` (or `/vue`) |
| `PluginHost`, `PluginHostProps`, `PluginHostRef`, `templatePlugin` | `@xcrong/docx-js-editor` | `@xcrong/docx-editor-react/plugin-api` |
...
Once the moved symbols are handled, every remaining `@xcrong/docx-js-editor` import changes to `@xcrong/docx-editor-react`. Only the package name changes; the imported names stay the same.
...
| `@xcrong/docx-js-editor/core` | `@xcrong/docx-editor-core` |
| `@xcrong/docx-js-editor/mcp` | `@xcrong/docx-editor-agents/mcp` |
| `@xcrong/docx-js-editor/i18n/<code>.json` | `@xcrong/docx-editor-i18n` (see [i18n restructure](#i18n-restructure) below) |
...
```diff
- import { FormattingBar } from '@xcrong/docx-js-editor';
+ import { Toolbar } from '@xcrong/docx-editor-react/ui';
```
...
```diff
- import { Toolbar } from '@xcrong/docx-js-editor';
+ import { EditorToolbar } from '@xcrong/docx-editor-react/ui';
// or compose your own:
+ import { Toolbar, ToolbarButton, ToolbarGroup } from '@xcrong/docx-editor-react/ui';
```
...
import { pl, ptBR } from '@xcrong/docx-editor-i18n';
```

The diff from 0.x:

```diff
- import pl from '@xcrong/docx-js-editor/i18n/pl.json';
+ import pl from '@xcrong/docx-editor-i18n/pl';
```
...
```diff
- import(`@xcrong/docx-js-editor/i18n/${locale}.json`).then(setI18n);
+ import(`@xcrong/docx-editor-i18n/${locale}`).then((m) => setI18n(m.default));
```
...
+ import type { LocaleStrings, PartialLocaleStrings } from '@xcrong/docx-editor-i18n';
+ type LocaleI18n = LocaleStrings | PartialLocaleStrings | undefined;
```

## License change

The 0.x line of `@xcrong/docx-editor-agents` was AGPL-3.0. In 1.0, every published package is **Apache 2.0**. If you tracked license obligations for compliance review, update the entry. Apache 2.0 is permissive: no copyleft on derivative works.

The non-agents packages (`-core`, `-react`, `-vue`, `-i18n`) were also previously AGPL through `@xcrong/docx-js-editor`; they're now Apache 2.0 as well.

## Vue users

Vue 3 is a published adapter in 1.0; it didn't exist in 0.x. If you were embedding the editor in a Vue app via the React adapter + a wrapper, drop the wrapper and use `@xcrong/docx-editor-vue` directly. The surface mirrors React (same component name, hooks-as-composables, same plugin contract). Cross-adapter parity is enforced by `scripts/parity/parity.contract.json` upstream. See the [Vue package page](/docs/1.x/vue).
...
If something behaves differently and isn't covered above, it's likely a bug rather than an intended change; please file an issue in the [docx-editor repo](https://github.com/xcrong/docx-editor/issues).
