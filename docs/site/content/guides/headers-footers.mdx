...
`getDocumentWatermark` and `setDocumentWatermark` from `@xcrong/docx-editor-core/headless` read and write the same model:

```ts
import { parseDocx, setDocumentWatermark, repackDocx } from "@xcrong/docx-editor-core/headless";
