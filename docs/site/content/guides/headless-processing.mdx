...
description: 'Parse, inspect, mutate, and serialize DOCX files in Node.js or workers with @xcrong/docx-editor-core/headless. No DOM and no editor UI required.'
category: 'Guides'
seoTitle: 'Headless DOCX processing'
---

`@xcrong/docx-editor-core/headless` is the document engine without the editor UI: the OOXML parser, document model, mutation helpers, and serializer. It runs in Node.js and web workers. Nothing in it touches the DOM.
...
```ts
import { createDocumentWithText, createDocx } from "@xcrong/docx-editor-core/headless";
...
```ts
import { detectVariables, parseDocx, processTemplate } from "@xcrong/docx-editor-core/headless";
...
- **`DocxReviewer`** from `@xcrong/docx-editor-agents` wraps this same engine in an LLM tool surface: an agent reads the document, decides what to change, and applies edits (including tracked changes) through tool calls. Use it when the transformation requires judgment. See [DocxReviewer](/docs/1.x/agents/docx-reviewer).
