...
import { DocxEditor, type EditorMode } from '@xcrong/docx-editor-react';
...
The accept/reject operations behind the sidebar buttons are exported as ProseMirror commands from `@xcrong/docx-editor-core/prosemirror/commands`:
...
To enumerate revisions (for your own list UI or an approval workflow), use `extractTrackedChanges` from `@xcrong/docx-editor-core/prosemirror/utils/extractTrackedChanges`. It takes the editor state and returns the revision entries plus a comment-to-revision map.
