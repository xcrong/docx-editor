...
For host code that isn't using the React or Vue adapter, `@xcrong/docx-editor-core/utils` exports `loadFontFromUrl`, `loadFontDefinitions`, `onFontError`, and the `FontDefinition` type so you can drive the same registration directly.
...
| Prop | Type | Description |
|---|---|---|
| `i18n` | `LocaleStrings \| PartialLocaleStrings` | Locale data from `@xcrong/docx-editor-i18n`. See [i18n page](/docs/1.x/i18n). |
