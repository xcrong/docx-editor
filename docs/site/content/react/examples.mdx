...
Hide the built-in toolbar and compose the primitives from `/ui`. `ToolbarButton` takes `active`, `onClick`, and children; drive both from the editor ref and `onSelectionChange`:
...
Prefer the prebuilt strip? Keep `showToolbar` on and append your own controls with `toolbarExtra`, or compose the full `EditorToolbar` chrome; both are covered in [Toolbar customization](/docs/1.x/guides/toolbar).
