...
`McpServer` exposes the [14 agent tools](/docs/1.x/agents/tools) over the Model Context Protocol, so any MCP-compatible client (Claude Desktop, agent platforms, your own MCP client) can read, comment, and redline a DOCX without knowing anything about this library.
...
- Never throws. Protocol-level problems come back as JSON-RPC errors; tool failures come back as successful responses with `isError: true` in the MCP content payload, per MCP convention.
...
- Building a server pipeline you also own: calling [DocxReviewer](/docs/1.x/agents/docx-reviewer) directly is less moving parts than JSON-RPC framing.
