...
Reference for wiring an agent to a running `<DocxEditor>`. For the end-to-end walkthrough, read the [AI editing tutorial](/docs/1.x/agents/ai-editing) first; this page documents the surface it builds on.

## useDocxAgentTools (React)

The full-featured hook from `@eigenpal/docx-editor-agents/react`. It owns the bridge to the editor and returns everything a chat integration needs.
...
- `include` / `exclude` filter built-in tools both in the returned schemas and at execution time. A filtered tool call returns `Tool 'x' is not enabled.`, so a hallucinated name cannot bypass the policy. Custom tools always pass.
...
`<DocxEditor agentPanel={...}>` mounts a right-hand panel with library-owned chrome (header, close button, drag-resize) and a toolbar toggle button. Three control patterns:
...
Panel chrome is intentionally not customizable beyond these options; everything inside `render` is yours. In Vue the `agentPanel` prop does not exist yet (React-only per the parity contract); mount the `AgentPanel` component from `@eigenpal/docx-editor-agents/vue` next to the editor instead.

## Chat UI primitives

Optional, opinionated components for the inside of the panel. React: `@eigenpal/docx-editor-agents/react`. Vue: `@eigenpal/docx-editor-agents/vue`.
...
If you drive the chat with the Vercel AI SDK, `toAgentMessages(chat.messages, chat.status)` from `@eigenpal/docx-editor-agents/ai-sdk/react` (or `/ai-sdk/vue`) converts `useChat`'s `UIMessage[]` into the `AgentMessage[]` shape these components render. Nothing forces these components: the panel accepts any node, so assistant-ui or your design system drop in.
...
- Paragraphs that arrive without a `paraId` (Word does not always emit them) are addressed by their ordinal index as a string. The ids the agent reads always resolve when written back.
