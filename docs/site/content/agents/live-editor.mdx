...
The main React hook from `@xcrong/docx-editor-agents/react`. It creates the bridge to the editor and returns the tool schemas, executor, and context snapshot.
...
The Vue twin, from `@xcrong/docx-editor-agents/vue`:

```ts
import { ref } from 'vue';
import { useAgentBridge, type EditorRefLike } from '@xcrong/docx-editor-agents/vue';
...
For hosts that are neither React nor Vue, build the bridge directly from `@xcrong/docx-editor-agents/bridge`:

```ts
import { createEditorBridge, type EditorRefLike } from '@xcrong/docx-editor-agents/bridge';
import { executeToolCall, getToolSchemas } from '@xcrong/docx-editor-agents';
...
The panel shell is configurable through these options; render your own chat UI inside `render`. In Vue the `agentPanel` prop does not exist yet (React-only per the parity contract); mount the `AgentPanel` component from `@xcrong/docx-editor-agents/vue` next to the editor instead.

## Chat UI components

Optional components for the inside of the panel. React: `@xcrong/docx-editor-agents/react`. Vue: `@xcrong/docx-editor-agents/vue`.
