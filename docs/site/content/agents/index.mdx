---
title: 'Overview'
description: 'Agent toolkit for DOCX: 14 tools for AI comments, tracked changes, and redlining. Run them against a live editor, headless in Node, or over MCP.'
category: 'Agents'
order: 50
...
`@eigenpal/docx-editor-agents` gives an LLM a Word-like API for DOCX documents: read paragraphs, add comments, suggest tracked changes, apply formatting. The same 14 tools run in three places. You bring your own agent runtime (Vercel AI SDK, OpenAI, Anthropic, LangChain); the toolkit ships plain JSON-Schema tool definitions plus optional adapters.
...
**1. Live editor panel.** The agent's tool calls land on a `<DocxEditor>` running in the user's browser. Comments and tracked changes appear in the document as the model streams. Tool calls execute in the browser against the open editor; your API route only ever sees tool-call text.
...
<Cards>
  <Card title="AI editing tutorial" href="/docs/1.x/agents/ai-editing">
    Wire an LLM to the live editor with the Vercel AI SDK, end to end.
  </Card>
