...
// resolve `@xcrong/docx-editor-vue` + `/agents/*` through node_modules
// (the workspace's published dist/) so the deployment shows the real
// consumer experience. Core deep paths (e.g.
// `@xcrong/docx-editor-core/headless`) are kept aliased to source in
...
const coreAliases = [
  ...
  // Exact match for bare @xcrong/docx-editor-core (must come AFTER prefix match)
  {
    find: /^@xcrong\/docx-editor-core$/,
  ...
  resolve: {
    alias: usePublished
      ...
      : [
          ...
          {
            find: '@xcrong/docx-editor-agents/vue',
            replacement: path.join(monorepoRoot, 'packages/agents/src/vue.ts'),
          },
          {
            find: '@xcrong/docx-editor-agents/bridge',
            replacement: path.join(monorepoRoot, 'packages/agents/src/bridge.ts'),
          },
          // Bare @xcrong/docx-editor-agents (e.g. for type re-exports)
          {
